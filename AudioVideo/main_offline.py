...
if __name__ == "__main__":
    ...
    videoFileName = 'AudioVideo/data/Vid_18_Nocturne_vn_fl_tpt 5.mp4'
    ...
    with open('predicted_beat_time.csv', newline='') as f:
        reader = csv.reader(f)
        data = list(reader)

    cmd="ffmpeg -y -i '"+videoFileName+"'"
    for val in data:
        cmd=cmd+" -itsoffset "+str(float(val[0]))+" -i drum-a.wav "

    cmd=cmd+" -filter_complex amix=inputs="+str(len(data)+1)+"[a] -map 0:v -map [a] -c:v copy -async 1 -c:a aac -strict experimental output3.mp4"
    os.system(cmd)
